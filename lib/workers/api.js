 ...
 **/
 ...
 const collectStats = () => {
 	...
 	async.parallel({
        ...
        topminers: function (callback) {
            let startTime = process.hrtime.bigint();

            const limit = 10
            const data = []
            for(let address in minerStats) {
                if(!minerStats[address]['stats']) {
                    continue
                }
                const stat = minerStats[address]['stats'];
            ...
            const topMiners = {
                limit,
                 current_hashrate:data.length > 0 ? compareTopMiners(data.slice(), limit) : [],
                 donate:data.length > 0 ? compareTopDonators(data.slice(), limit) : [],
                 unblocker:data.length > 0 ? compareTopUnblockers(data.slice(), limit) : [],
                 hashes:data.length > 0 ? compareTopHashes(data.slice(), limit) : []
            }
            timelapse.topMiners =  process.hrtime.bigint() - startTime
            callback(null, topMiners);
        },
 ...
 */
 ...
 function collectWorkerStats(address, statsCallback) {
    if(poolStats && poolStats.pool && poolStats.pool.stats) {
        let yesterday = lastCollectWorkerStats - 86400
        while(poolStats.pool.stats.lastBlockFound > yesterday) {
            yesterday-= 86400
        }
        if(minerStats[address] && 
            minerStats[address]['stats'] && 
            minerStats[address]['stats']['lastShare'] && 
            (parseInt(minerStats[address]['stats']['lastShare']) < yesterday))
        {
 	...
 	async.waterfall([
        ...
        function(callback){
            ...
            if(!poolStats || !poolStats.network)  {
                Cache.clear(global.config.coin + ':blocks:candidates')
            }else if(poolStats.network.height !== lastHeight) {
                lastHeight = poolStats.network.height
                Cache.clear(global.config.coin + ':blocks:candidates')
            }

            Cache.readCallback(global.config.coin + ':blocks:candidates', cache => {
            	...
            	redisClient.zrevrange(config.coin + ':blocks:candidates', 0, -1, 'WITHSCORES', function(error, results){
                    ...
                    cache(blocks, (poolStats && poolStats.config && poolStats.config.coinDifficultyTarget? poolStats.config.coinDifficultyTarget : 120) * 1000)// base on block time
...
function storeCollectedValues(chartName, values, settings) {
    for(let [k,v] of Object.entries(values)) {
        storeCollectedValue(chartName + ':' + k, v, settings);
    }
}
    ...
    next => {
        ...
        async.parallel({
            ...
            function(eee, rrr) {
                ...
                for(let [k,v] of Object.entries(rrr)) {
