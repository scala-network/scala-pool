 ...
 */
 ...
 const PoolRedirect = require('./model/PoolRedirect.js');
 ...
 **/ 
 ...
 process.on('message', function(message) {
    switch (message.type) {
        ...
        break;
        case 'blockTemplate':
        // log('info', logSystem, 'Block Template recieved for height %s', message.block.height);
        if(!poolStarted) {
            startPoolServerTcp(() => {
...
Miner.prototype = {
    ...
    getJob:function(){
        ...
        let blob = this.agent.isProxy ? currentBlockTemplate.nextBlobWithChildNonce() : currentBlockTemplate.nextBlob();
 ...
 // function handleMinerMethod(req, res){
    ...
    switch(method){
        ...
        if (!addressType) {
           ...
           log('warn', logSystem, 'Invalid address used for login (prefix: %s): %s@%s', [addressPrefix, login, ip]);
        ...
        if(!PoolRedirect.isEnabled() || (miner.agent.name.toLowerCase() !== 'xlarig') || (miner.agent.versionInt < 5202)) {
        	noRedirectAction();
		break;
	}

        return PoolRedirect.byMiner(miner, "onLogin", (error, response) => {
            
            if(error || !response.redirect) {
    		    return noRedirectAction();
    	    }
    ...
    const rSendReply = (e,r) => {

        if(!PoolRedirect.isEnabled() || (miner.agent.name.toLowerCase() !== 'xlarig') || (miner.agent.versionInt < 5202)) {
            return sendReply(e,r);
        }
        
        PoolRedirect.byMiner(miner, "onMining", (error, response) => {
        
            if(!error && response && response.redirect) {
                r.redirect = response.redirect;
            }
    ...
    if (!job){
        rSendReply('Invalid job id',miner);
        return;
    }

    if (!params.nonce || !params.result) {
        rSendReply('Attack detected',miner);
    ...
    if (!noncePattern.test(params.nonce)) {
        ...
        rSendReply('Duplicate share',miner);
        return;
    }
        ...
        if (!blockTemplate){
            rSendReply('Block expired',miner);
            return;
        }
        ...
        if (!shareAccepted){
            rSendReply('Rejected share: invalid result',miner);
            return;
        }
 ...
 **/
 function processShare(miner, job, blockTemplate, params){
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        if(hard_fork_version >= 13 && job.height % 4 === 0) {
...
function poolStart(callback){
    ...
    if (portData.ssl) {
        ...
        net.createServer(socketResponder).listen(portData.port,'0.0.0.0', function (error, result) {
            if (error) {
