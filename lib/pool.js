...
global.EventManager.register('pool:server:created',{
    ...
    cmds.push(['hincrby', global.coin + ':stats', 'workers', '+1']);
    cmds.push(['hincrby', global.coin + ':stats', workerPortType, '+1']);
    cmds.push(['hincrby', global.coin + ':stats', porto, '+1']);
    cmds.push(['hmget', global.coin + ':stats', ['workers',workerPortType ]]);
    ...
    redisClient.multi(cmds).exec((e,r) => {
        ...
        re.push(['hset', global.coin + ':graph:connects:all', dateNowSeconds, rr[0]]);
        re.push(['hset', global.coin + ':graph:connects:' + miner.poolType, dateNowSeconds, rr[1]]);
 ...
 // function handleMinerMethod(req, res){
    ...
    switch(method){
        ...
        if(addr.length >= 2) {
    	    ...
    	    paymentId = addr[1]
            paymentId = paymentId.replace(/\s/g, '');
    	    
	        if(!utils.hasValidPaymentId(paymentId)){
                return sendReply('Invalid payment id used for login');
            }

            login = address + '.' + paymentId
        } else {
            login = address
        }
 ...
 **/
 function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    const redisCommands = []
    if(poolType === 'solo') {
        redisCommands.push(['hincrby', coin + ':' +  poolType + ':shares_actual:roundCurrent', login + '_score', score])
        redisCommands.push(['hincrby', coin + ':' +  poolType + ':shares_actual:roundCurrent', login + '_donations', donations])
    } else {
        redisCommands.push(['hincrby', coin + ':' + poolType + ':shares_actual:roundCurrent', login, score])
        redisCommands.push(['hincrby', coin + ':' + poolType + ':shares_actual:roundCurrent', 'donations', donations])
        redisCommands.push(['hincrby', coin + ':' + poolType + ':shares_actual:roundCurrent', 'total', diff])
    }
    ...
    redisCommands.push(['hincrby', coin + ':workers:' + login, 'hashes', score])
    redisCommands.push(['hset', coin + ':workers:' + login, 'lastShare', dateNowSeconds])
    redisCommands.push(['hset', coin + ':workers:' + login, 'lastShare', dateNowSeconds])
    redisCommands.push(["hincrby",coin + ':' + poolType + ':block_donations:current',login,donations])
    ...
    if (blockCandidate){
        ...
        if(poolType === 'solo') {
            redisCommands.push(['hmget', coin + ':' + poolType + ':shares_actual:roundCurrent', [login + '_score',  login + '_donations']])
            redisCommands.push(['hmset', coin + ':' + poolType + ':shares_actual:roundCurrent', [login + '_score',0,  login + '_donations',0]])
        } else {
            redisCommands.push(["rename",coin + ':' + poolType + ':block_donations:current',coin + ':block_donations:round' + job.height])
            redisCommands.push(['rename', coin + ':' + poolType + ':shares_actual:roundCurrent', coin + ':shares_actual:round' + job.height]);
    ...
    redisClient.multi(redisCommands).exec(function(err, replies){
        ...
        if (blockCandidate){
            ...
            const redCmd = []
           if(poolType === 'solo') {
                totalShares = replies[replies.length -2][0]
                donate = replies[replies.length -2][1]
                redCmd.push(['hmset', coin + ':shares_actual:round' + job.height,[
                        login,totalShares,
                        'donations',donate,
                        'total', totalShares + donate
                    ]
                ])

            } else {
                totalShares = replies[replies.length -1][0]
                donate = replies[replies.length -1][1]
            }
            ...
            redisClient.multi(redCmd).exec(function(err2, replies2){
                if (err){
