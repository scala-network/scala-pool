...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage)
{
 // function handleMinerMethod(req, res){
    ...
    switch(method){
        ...
        PoolRedirect.byMiner(miner, "onLogin", (error, response) => {
            
            if(error || !response.redirect) return noRedirectAction();
    ...
    if (!noncePattern.test(params.nonce)) {
        ...
        case 'keepalived' :
            ...
            const noRedirectActionOnMining = () => {
                if (!miner){
                    sendReply('Unauthenticated');
                    return;
                }
            ...
            if(!PoolRedirect.isEnabled() || (miner.agent.name.toLowerCase() !== 'xlarig') || (miner.agent.versionInt < 5202)) {
                return noRedirectActionOnMining();
            }
            
            PoolRedirect.byMiner(miner, "onMining", (error, response) => {
            
                if(error  || !response.redirect) return noRedirectActionOnMining();
                sendReply(null, {
                    status: 'KEEPALIVED',
                    redirect: response.redirect
                });
