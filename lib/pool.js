...
// Set redis database cleanup interval by days
let cleanupInterval = config.redis.cleanupInterval && config.redis.cleanupInterval > 0 ? config.redis.cleanupInterval : 0;
let cleanupMinerInterval = 0;
let cleanupPaymentInterval = 0;
if(config.redis.cleanup) {
	cleanupMinerInterval = config.redis.cleanup.miner || cleanupInterval;
	cleanupPaymentInterval = config.redis.cleanup.payment || cleanupInterval;
} else {
	cleanupMinerInterval = cleanupInterval;
	cleanupPaymentInterval = cleanupInterval;
...
Miner.prototype = {
    ...
    getJob: function(){
        if (currentBlockTemplate && this.lastBlockHeight === currentBlockTemplate.height && !this.pendingDifficulty && this.cachedJob !== null) {
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    let redisCommands = [
        ...
        ['expire', config.coin + ':workers:' + miner.login, (86400 * cleanupMinerInterval)],
        ['expire', config.coin + ':payments:' + miner.login, (86400 * cleanupPaymentInterval)]
    ];
    ...
    if (miner.workerName) {
        ...
        redisCommands.push(['expire', config.coin + ':unique_workers:' + miner.login + '~' + miner.workerName, (86400 * cleanupMinerInterval)]);
    }
	...
	if(replies.length <= 0){
	    log('info', logSystem, 'Submitted score ' + job.score + ' for difficulty ' + job.difficulty + ' and no replies ');
		return;
	}
        if (slushMiningEnabled) {
...
function processShare(miner, job, blockTemplate, params){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        convertedBlob = utils.cnUtil.convert_blob(shareBuffer, cnBlobType);
       	hash = cnHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'),cnVariant);
        shareType = 'valid';
    }
...
function startPoolServerTcp(callback){
	...
	const bindAddress = config.poolServer.bind || "0.0.0.0";
...
//                            if (e.message) log('warn', logSystem, 'Exception: ' + e.message);
        ...
        if (portData.ssl) {
            if (!config.poolServer.sslCert) {
                ...
                tls.createServer(options, socketResponder).listen(portData.port,bindAddress, function (error, result) {
                    if (error) {
                        log('error', logSystem, 'Could not start server bind address %s on port %d (SSL), error: $j', [bindAddress, portData.port, error]);
                        cback(true);
                        return;
                    }
            ...
            net.createServer(socketResponder).listen(portData.port, bindAddress, function (error, result) {
                if (error) {
                    log('error', logSystem, 'Could not start server bind address %s on port %d, error: $j', [bindAddress, portData.port, error]);
                    cback(true);
                    return;
                }
