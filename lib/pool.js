...
// Pool settings
const cleanupInterval = config.redis.cleanupInterval && config.redis.cleanupInterval > 0 ? config.redis.cleanupInterval : 30;
...
process.on('message', function(message) {
    switch (message.type) {
        case 'banIP':
        bannedIPs.push(message.ip)
        break
        case 'banIPs':
        bannedIPs = message.ips
        break
        case 'blockTemplate':
        if(!poolStarted) {
            startPoolServerTcp(() => {
 ...
 **/
 ...
 // function handleMinerMethod(req, res){
    ...
    if (IsBannedIp(ip))  return sendReply('Your IP is banned',miner)
 ...
 **/
 function IsBannedIp(ip){
    if (!banningEnabled) return false;

    return !!~bannedIPs.indexOf(ip);
}
 ...
 **/
 function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    redisCommands.push(['expire', coin + ':workers:' + workerAlias, (86400 * 30)])
    ...
    if (workerAlias) {
        ...
        redisCommands.push(['expire', coin + ':unique_workers:' + workerAlias, (86400 * 7)])
    }
