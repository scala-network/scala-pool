  ...
  async.waterfall([
        ...
        if (block.workerShares) {
          ...
          for(let a=0;a<Object.keys(block.workerShares).length;a++) {
            ...
            if(haveBlockUnlockerAward && worker === block.miner){
              ...
              blockStats.addInfo(block.height,worker,"unlockReward",unblockerAward);
              ...
              blockStats.addInfo(block.height,worker,"unlockReward", 0);
              log('info', logSystem, '-- %s | %d%% | %s | 0.00', [worker, percent*100, parseFloat(workerReward/100).toFixed(2)]);    

            }

            blockStats.addInfo(block.height, worker,'shares',share);
    ...
    **/
    (blockStats, callback) => {
      ...
      const cmdRedis = [];
      const heights = blockStats.getHeights();
      ...
      redisClient.multi(cmdRedis).exec((e,r) => {
        if(e) {
          log('error', logSystem, 'Error with fetching blocks %j', [e]);
          callback(null,finalCommandments);
          return;
        }
        ...
        for(let i=0;i<r.length;i+=2) {
          ...
          if(r[i]) {
            ...
            for(let a=0;a<donors.length;a++ ) {
              ...
              blockStats.addInfo(height, wallet,'donations',donate);
            }
          } 
          h++
        }
