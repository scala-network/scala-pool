                ...
                apiInterfaces.rpcWallet(rpcCommand, rpcRequest, function(error, result){
                   ...
                   result.fee_list.map(function(k){
                    ...
                    for (var i = 0; i < transferCommand.rpc.destinations.length; i++){
                        ...
                        transferCommand.redis.push(['hincrbyfloat', config.coin + ':workers:' + worker, 'balance', -deductAmount]);
                        transferCommand.redis.push(['hincrbyfloat', config.coin + ':workers:' + worker, 'paid', deductAmount]);
