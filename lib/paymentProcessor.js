        ...
        function(callback) {
            ...
            executeTransfer(payee, error => {
                if(utils.validateMinerAddress(error)) {
                    invalidPaymentAddress.push(payee)
                }
                callback(error);
            });
        }], function(e) {
        ...
        function(keys, callback){
            ...
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                for (var i = 0; i < replies.length; i++){
                    if(!!~invalidPaymentAddress.indexOf(keys[i])) {
			continue;
		    }
			...
			log('error', logSystem, "Invalid parts saved %s", [keys[i]]);
                        invalidPaymentAddress.push(keys[i])
			continue;
		    }
