...
const notifications = require('./notifications.js');utils = require('./utils.js');
...
function runInterval(){
	...
	function(balances, minPayoutLevel, callback) {
		...
		apiInterfaces.rpcWallet('getbalance',{},function(e,r){
			if(e){
				return callback(e);
			}
			callback(null,balances, minPayoutLevel,r.unlocked_balance);
		});
	},
        // Filter workers under balance threshold for payment
        function(balances, minPayoutLevel, walletBalance, callback){
            var payments = {};
	    let currentPayout = 0;
		...
		if(payout > (walletBalance * 0.5)){
			payout = payout * 0.5;
			if(payout > (walletBalance * 0.5)) {
				continue;
			}
		}
	    	  if((payout + currentPayout) > walletBalance) {
			break;
		  } 
	...
	if(currentPayout > walletBalance) {
		  callback(true);
		return;
	} 

            log('info', logSystem, '%d workers\' balances reached the minimum payment threshold : %d bank balance : %d',[totalOfPayments, currentPayout, walletBalance]);
