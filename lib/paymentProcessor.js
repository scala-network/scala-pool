        ...
        function(keys, callback){
            ...
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                for (var i = 0; i < replies.length; i++){
                    ...
                    if(!!~invalidPaymentAddress.indexOf(workerId)){
                      log('info', logSystem, 'Address invalid unable to make payment to :  %s', [workerId]);
                      continue;
                  }
                  const miner_type = utils.validateMinerAddress(workerId)
                  if(!miner_type) {
                ...
                switch(miner_type) {
                    case 2:
                    case 3:
                    defaultLevel = global.config.payments.minPaymentExchangeAddress || global.config.payments.minPaymentIntegratedAddress || global.config.payments.minPayment || 0
                    break
                    case 4:
                    defaultLevel = global.config.payments.minPaymentSubAddress || global.config.payments.minPaymentExchangeAddress || global.config.payments.minPayment || 0
                    default:
                    break
                }
                ...
                if(global.config.payments.minerPayFee && !config.payments.dynamicTransferFee){
