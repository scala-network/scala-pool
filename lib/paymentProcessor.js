 ...
 **/
 ...
 const minLevelIntegrated = global.config.payments.minExchangeAddress;
        ...
        function(keys, callback){
            ...
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                for (var i = 0; i < replies.length; i++){
                    ...
                    if(!addrType) {
                        log('info', logSystem, 'Address is invalid :  %s', [workerId]);
                    ...
                    if(addrType === 3){
                        ...
                        if(addr.length < 2 || ~utils.hasValidPaymentId(addr[1])){
                            invalidPaymentAddress.push(workerId)
                            log('info', logSystem, 'Address has invalid payment id :  %s', [workerId]);
                            continue
