        ...
        function(balances, minPayoutLevel, callback){
            ...
            for (var worker in payments){
                ...
                if(isIntegratedAddress){
                    ...
                    tci.redis.push(['hincrbyfloat', config.coin + ':workers:' + worker, 'balance', -amount]);
                    tci.fee[address] = 0;
                    if(config.payments.minerPayFee){
                        if(!config.payments.dynamicTransferFee){
                            tci.redis.push(['hincrbyfloat', config.coin + ':workers:' + worker, 'balance', -config.payments.transferFee]);
