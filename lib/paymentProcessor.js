...
function runInterval(){
	...
	var minLevelStellitePay = config.payments.minPaymentStellitePay || minLevelIntegrated;
    ...
    async.waterfall([
            ...
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                for (var i = 0; i < replies.length; i++){
		    		...
		    		if(invalidPaymentAddress.indexOf(workerId) >= 0){
						log('warning', logSystem, 'Address invalid unable to make payment to :  %s', [workerId]);
						continue;
		    		}
        ...
        function(keys, balances, callback){
			var redisCommands = keys.map(function(k){
                return ['hget', k, 'isStellitePay'];
            });
            redisClient.multi(redisCommands).exec(function(error, replies){
				if (error){
                    log('error', logSystem, 'Error with stellitepay user redis %j', [error]);
                    return callback(null, keys, balances, []);
                }	
				const stellitePayUsers = [];
				for (var i = 0; i < replies.length; i++){
                    var parts = keys[i].split(':');
                    var wallet = parts[parts.length - 1];
					if(replies[i] === 1){
						stellitePayUsers.push(wallet);
					}
				 }
				 callback(null, keys, balances, stellitePayUsers);
			});
		},
        // Get worker minimum payout
        function(keys, balances, stellitePayUsers, callback){
            var redisCommands = keys.map(function(k){
            ...
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                for (var i = 0; i < replies.length; i++){
					...
					if(stellitePayUsers.indexOf(wallet) >= 0){
                        minLevel = minLevelStellitePay;
                        defaultLevel = minLevelStellitePay;						
					} else if(utils.isIntegratedAddress(wallet)){
        ...
        function(balances, minPayoutLevel, callback){
            ...
            for (var wallet in payments){
                ...
                if(isIntegratedAddress){
                    ...
                    if(config.payments.minerPayFee){
                        if(config.payments.dynamicTransferFee){
                            ...
                            tci.fee[wallet] = config.payments.transferFee;
                ...
                transferCommands[commandIndex].fee[wallet] = 0;
            ...
            function executeTransfer(transferCommand,cback){
                ...
                apiInterfaces.rpcWallet(rpcCommand, rpcRequest, function(error, result){
                    ...
                    for (var i = 0; i < transferCommand.rpc.destinations.length; i++){
                        ...
                        transferCommand.redis.push(['hincrbyfloat', config.coin + ':workers:' + wallet, 'balance', -deductAmount]);
                    ...
                    log('info', logSystem, 'Payments sent via wallet daemon %j', [result || null]);
