...
const executeTransfer = (transferCommand,callback) => {
    ...
    apiInterfaces.rpcWallet(rpcCommand, rpcRequest, function(error, result){
        ...
        for (let i = 0; i < transferCommand.rpc.destinations.length; i++){
            const destination = transferCommand.rpc.destinations[i];
            const wallet = destination.address;

            if (transferCommand.rpc.payment_id){
                wallet += global.config.poolServer.paymentId.addressSeparator + transferCommand.rpc.payment_id;
            }
