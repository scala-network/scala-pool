...
exports.getMinerPayoutLevel=function(urlParts, sendData) {
    ...
    redisClient.hget(config.coin + ':workers:' + address, 'minPayoutLevel', function(error, value) {
		...
		switch(addressType){
            case 2:
            case 3:
            const min = global.config.payments.minPaymentExchangeAddress || global.config.payments.minPaymentIntegratedAddress || config.payments.minPayment
			minLevel = (min / config.coinUnits) || (config.payments.minPayment / config.coinUnits) || 0;
            break
            case 4:
            const min = global.config.payments.minPaymenSubAddress || config.payments.minPayment
            minLevel = (min / config.coinUnits) || (config.payments.minPayment / config.coinUnits) || 0;
		    default:
            break;
		}
...
exports.setMinerPayoutLevel=function(urlParts, sendData){
    ...
    utils.minerSeenWithIPForAddress(address, ip, function (error, found) {
        if (!found || error) {
          sendData({status:'error',message: 'We haven\'t seen that IP for your address', level: level,min:minLevel,max:maxLevel});
          return;
        }
